...
DlibCase::~DlibCase()
{
	Logger->info("DlibCase::~DlibCase()");
}

void DlibCase::loadFromConfig(QJsonObject const& a_config)
{
...
void DlibCase::loadDronConfigs(QJsonArray const& a_preprocess)
{
	m_dronNoise = 100;
	m_dronContrast = 100;
...
void DlibCase::loadDatasetName(QJsonObject const& a_dataset)
{
	#ifdef _WIN32
	QJsonObject jDataset{ a_dataset[DATASET_WIN32].toObject() };
	#endif // _WIN32
	#ifdef __linux__ 
	QJsonObject jDataset{ a_dataset[DATASET_UNIX].toObject() };
	#endif // _UNIX
...
void DlibCase::configure(QJsonObject const& a_config, QJsonArray const& a_preprocess, QJsonObject const& a_dataset, QJsonArray const& a_postprocess)
{
	#ifdef DEBUG
	Logger->debug("DlibCase::configure()");
	#endif
	m_config = a_config;
	m_postprocess = a_postprocess;
	
	#ifdef DEBUG
	qDebug() << "DlibCase::configure() m_postprocess:" << m_postprocess << "\n";
	#endif
	DlibCase::loadFromConfig(a_config);
	DlibCase::loadDronConfigs(a_preprocess);
	...
	std::vector<image_info> listing = m_dataMemory->get_imageInfoTrain();
	...
	Logger->debug("Dlib::configure() m_dataMemory->get_imageInfoTrain() ok");
	#endif
...
void DlibCase::testNetwork(net_type segb)
{
	...
	{
		#ifdef DEBUG
		//Logger->debug("DlibCase::loadNetwork() Load the input image:");
		#endif
		...
		#ifdef DEBUG
		//Logger->debug("DlibCase::loadNetwork() instances nr:{} nc:{}", input_image.nr(), input_image.nc());
		//Logger->debug("DlibCase::loadNetwork() instances nr:{} nc:{}", label_image.nr(), label_image.nc());
		#endif
...
void DlibCase::postprocessing()
{
	...
	Logger->debug("DlibCase::postprocessing() m_postprocess.size():{}", m_postprocess.size());
	#endif
	for (int i = 0; i < m_postprocess.size(); i++)
	{
		#ifdef DEBUG_POSTPROCESSING
		qDebug() << "DlibCase::postprocessing() m_postprocess[i]:" << m_postprocess[i];
		...
		#ifdef DEBUG_POSTPROCESSING
		Logger->debug("DlibCase::postprocessing() postprocess i:{}, _prevActive.size:{}", i, _prevActive.size());
		#endif
		...
		{
			#ifdef DEBUG_POSTPROCESSING
			Logger->debug("DlibCase::postprocessing() postProcessing: block[{}]->process", i);
			#endif
...
fitness DlibCase::finishPostProcessing()
{
	#ifdef DEBUG_POSTPROCESSING
	Logger->debug("DlibCase::finishPostProcessing() Calculate fitness:");
	#endif
	...
	for (int i = 0; i < m_postprocess.size(); i++)
	{
		...
		{
			#ifdef DEBUG_POSTPROCESSING
			Logger->debug("DlibCase::finishPostProcessing() Calculate Fitness endProcess:");
			#endif
			m_blockPostprocess[i]->endProcess(m_dataPostprocess[i]);
			fs = m_dataPostprocess[i][0].fs;
			#ifdef DEBUG_POSTPROCESSING
			Logger->debug("DlibCase::finishPostProcessing() fs:{}", fs.fitness);
			#endif
		}
		if (_type == "Encoder")
		{
			#ifdef DEBUG_POSTPROCESSING
			Logger->debug("DlibCase::finishPostProcessing() Calculate Encoder endProcess:");
			#endif
			m_blockPostprocess[i]->endProcess(m_dataPostprocess[i]);
		}
	}
...
truth_instance DlibCase::load_truth_instances(const image_info& info)
{
	#ifdef DEBUG
	Logger->debug("DlibCase::load_truth_instances({},{})", info.image_filename, info.gt_filename);
	#endif
	...
	#ifdef DEBUG
	Logger->debug("DlibCase::load_truth_instances({}x{})", nr, nc);
	#endif

	return ti;
}

void DlibCase::process(){}


void DlibCase::onSignalOk(struct fitness fs, qint32 slot)
{
}

void DlibCase::clearData()
{
}
...
net_type DlibCase::train_segmentation_network(const std::vector<truth_instance>& truth_images)
{
	...
	#ifdef DEBUG
	Logger->debug("DlibCase::train_segmentation_network() synchronization_file_name:{}", m_synchronizationNetworkFileName.toStdString());
	#endif
	std::vector<int> cuda{ 0 };
	if (m_useTwoCuda)
	{
		#ifdef DEBUG
		Logger->debug("DlibCase::train_segmentation_network() useTwoCUDA");
		#endif
		cuda.push_back(1);
	}
	#ifdef DEBUG
	Logger->debug("DlibCase::train_segmentation_network() create seg_trainer:");
	#endif
	...
	#ifdef DEBUG
	Logger->debug("DlibCase::train_segmentation_network() setting seg_trainer:");
	#endif
	...
	#ifdef DEBUG
	Logger->debug("DlibCase::train_segmentation_network() loop:");
	#endif
	...
	#ifdef DEBUG
	Logger->debug("DlibCase::train_segmentation_network() start threads... ");
	#endif
